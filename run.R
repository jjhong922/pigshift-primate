...
for (name in c("female-br", "female-cb", "female-ht", "female-kd", "male-br", "male-ht", "male-kd", "male-lv")){
    ...
    write.table(results$wAIC, sprintf('output/primate-%s/waic.tsv',name), sep = "\t");
    write.table(results$alpha, sprintf('output/primate-%s/alpha.tsv',name), sep = "\t");
    write.table(results$beta, sprintf('output/primate-%s/beta.tsv',name), sep = "\t");
    write.table(results$shift, sprintf('output/primate-%s/shift.tsv',name), sep = "\t");
}
